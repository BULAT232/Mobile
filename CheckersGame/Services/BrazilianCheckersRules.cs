...
/// Те же правила, что и международные, но на доске 8×8.
/// Летающие дамки, шашки бьют назад.
/// </summary>
public class BrazilianCheckersRules : ICheckersRules
{
    public int BoardSize => 8;
    public GameType GameType => GameType.Brazilian;
                ...
                if (midPiece != null && midPiece.Color != piece.Color)
                {
                    for (int landDist = 1; landDist < BoardSize; landDist++)
                    {
                        ...
                        if (board[landRow][landCol] != null &&
                            !(landRow == startRow && landCol == startCol)) break;
    ...
    public bool MustCapture(PieceModel?[][] board, PieceColor color) => HasAnyCapturesForColor(board, color);
    ...
    {
        return GetValidMoves(board, currentTurn).Count == 0;
    }

    public GameStatus GetGameResult(PieceModel?[][] board, PieceColor currentTurn)
    {
        if (!IsGameOver(board, currentTurn)) return GameStatus.InProgress;
        return currentTurn == PieceColor.White ? GameStatus.BlackWins : GameStatus.WhiteWins;
    }

    private bool IsInBounds(int row, int col) => row >= 0 && row < BoardSize && col >= 0 && col < BoardSize;
}
